...
TEST(MessageTests, DefaultConstructorValues)
{
    ...
    EXPECT_EQ(msg.get_type(), "") << "Expected default message type to be empty";
    EXPECT_EQ(msg.get_sender(), "") << "Expected default sender to be empty";
    EXPECT_EQ(msg.get_receiver(), "") << "Expected default receiver to be empty";
    std::string time = std::ctime(&now);
    time.erase(time.find_last_not_of(" \n\r\t") + 1); // trim the string off spaces and "\n\r\t"
    EXPECT_EQ(msg.get_time(), time) << "Expected message time to match current time";
}
...
TEST(MessageTests, TextMessageConstructorValues)
{
    ...
    EXPECT_EQ(textmsg.get_type(), "text") << "Expected message type to be 'text'";
...
TEST(VoiceMessageTests, VoiceMessageAttributes)
{
    ...
    EXPECT_EQ(voicemsg.get_sender(), "david") << "Expected sender to be 'david'";
...
TEST(UserTests, UserInitialization)
{
    ...
    EXPECT_EQ(user.get_username(), "david") << "Expected username to be 'david'";
}
...
TEST(ServerTests, ServerInitialState)
{
    ...
    EXPECT_EQ(server.get_public_keys().size(), 0) << "Expected no public keys initially";
    EXPECT_EQ(server.get_messages().size(), 0) << "Expected no messages initially";
}
...
TEST(ServerMessageCreationTests, CreateMessageWithCorrectSignature)
{
    // Test message creation with an correct digital signature
    Server server;
    User david = server.create_user("david");
    User jenifer = server.create_user("jenifer");

    VoiceMessage *voicemsg = new VoiceMessage("david", "jenifer");
    std::string signature = crypto::signMessage(david.get_private_key(), "");
    bool result = server.create_message(voicemsg, signature);

    EXPECT_TRUE(result)
        << "Message creation with correct signature should succeed";
    EXPECT_EQ(server.get_messages().size(), 1)
        << "1 message should appear in the server";
}
...
{
    // Test sending multiple text messages and a voice message acrossdifferent
    // users
    Server server;
    User david = server.create_user("david");
    User jenifer = server.create_user("jenifer");
...
TEST(ServerMessagingAnalysisTests, GetMessagesToUser)
{
    ...
    EXPECT_EQ(msgs[0]->get_type(), "text") << "First message should be a text message";
}
...
TEST(VectorAnalysisTests, CountUniqueElementsAboveThreshold)
{
    ...
    EXPECT_EQ(mylib::count_unique_above(v1, 5), 4)
    ...
    EXPECT_EQ(mylib::count_unique_above(v2, 30), 6)
...
TEST(VectorManipulationTests, ReverseAndConcatenateVectorElements)
{
    ...
    EXPECT_EQ(mylib::vector_to_string(v2, '*'), "14*12*9*8*7*6*5*4*3*2*1")
        << "Vector elements should be reversed and concatenated with '*' ";
}

// Test suite for mylib::square_elements
TEST(SquareElementsTest, HandlesEmptyVector)
{
    std::vector<int> v{};
    mylib::square_elements(v);
    EXPECT_TRUE(v.empty());
}
...
TEST(SquareElementsTest, NoOverflowCheck)
{
    ...
    // overflow scenarios
    EXPECT_EQ(std::vector<long long>(v.begin(), v.end()), (std::vector<long long>{10000000000LL, 10000000000LL}));
    // This test might fail on some systems due to overflow
}
...
TEST(CopyIfEvenTest, HandlesEmptySource)
{
    ...
    std::vector<int> destination{1, 2, 3}; // Intentionally pre-filled to test clearing
    mylib::copy_if_even(source, destination);
