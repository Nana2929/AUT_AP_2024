...
public:
    ...
    // Setters for Bank::transfer operation 
    bool increase_balance(double amount);
    ...
    bool set_balance(double amount, std::string &owner_fingerprint);

    bool set_loan(double amount, std::string &owner_fingerprint);
    // Setter APIs for Bank operations
    bool set_owner(const Person *new_owner, size_t hashed_fingerprint);
    bool set_status(bool status, size_t hashed_fingerprint);
    bool set_exp_date(std::string &exp_date, size_t hashed_fingerprint);
